...
// Isaac Saca Monroy 
// 09/06/2023
// Programa main.cpp donde se programa el menú del servicio de streaming junto con sus respectivas funciones
// Se incluye el sistema de clase creadao para el proyecto 
...
void cargarDatos(vector<Video*> &videos, vector<Serie*> &series){
    ...
    if (archivoSeries.is_open()) {
        ...
        while (getline(archivoSeries, lineaLeida)) {
            ...
            series.push_back(new Serie(numeroSerie, nombreSerie)); // se crea un nuevo objeto de tipo Serie y se agrega a un vector de apuntadores
    ...
    if (archivoVideos.is_open()) {
        ...
        while (getline(archivoVideos, lineaLeida)){
            ...
            // Si la primera letra es p, se agrega un nuevo objeto de tipo película al vector de videos 
            // Si la primera letra es e, se agrag un nuevo objeto de tipo episodio al vector de videos
            // Adicionalmente, se agrega cada episodio a la serie que le corresponde
            // Se verifica si ya existe una serie para acomodar el episodio y si no, se crea una nueva
            if (tipo == 'p') {
                ss >> ID >> nombre >> genero >> duracion >> calificacion;
                videos.push_back(new Pelicula(ID, nombre, genero, duracion, calificacion));
                
            } else if (tipo == 'e') {

                string nombreSerie, EpisodioS, TemporadaS;
                int numEpisodio = 0, numTemporada = 0;
                ss >> ID >> nombre >> genero >> duracion >> calificacion >> nombreSerie >> EpisodioS >> TemporadaS;
                numEpisodio = stoi(EpisodioS.substr(1, EpisodioS.length()));
                numTemporada = stoi(TemporadaS.substr(1, TemporadaS.length()));
                videos.push_back(new Episodio(ID, nombre, genero, duracion, calificacion, nombreSerie, EpisodioS, TemporadaS));
                ...
                for (int i = 0; i < series.size(); i++) {
                    if (series[i]->getTitulo() == nombreSerie) {
                        ...
                        series.back()->agregarEpisodio((Episodio*)videos[videos.size() - 1]);
                    }
                }
...
// Función para mostrar los videos en general con una cierta calificación o de un cierto género
void mostrarVideosCalificacionGenero(vector<Video*>& listaVideos, int calificacion, string genero) {
    cout << "Lista de Videos con Calificacion " << calificacion << " y Genero " << genero << ":" << endl;
    for (Video* video : listaVideos) {
        if (video->getCalificacion() == calificacion && video->getGenero() == genero) {
            video->imprimirInfo();
        }
    }
}

// Función para mostrar los episodios de una determinada serie con una calificación determinada
void mostrarEpisodiosSerie(vector<Serie*>&series, string titulo, int calificacion) {
    cout << "Episodios de la Serie " << titulo << " con Calificacion " << calificacion << ":" << endl;
    for (Serie* serie : series) {
...
void mostrarPeliculasCalificacion(vector<Video*>& listaVideos, int calificacion) {
...
void calificarVideo(vector<Video*>& listaVideos, string titulo, int valor) {
    for (Video* video : listaVideos) {
        if (video->getNombre() == titulo) {
            ...
            cout << "Se ha calificado el video \"" << titulo << "\" con valor " << valor << endl;
            return;
        }
    }
...
int main() {
    ...
    do {
        ...
        cout << "4. Mostrar los videos en general con una cierta calificacion o de un cierto genero" << endl;
        cout << "5. Mostrar los episodios de una determinada serie con una calificacion determinada" << endl;
        ...
        // Switch case que manda llamar a la funcion correspondiente según la elección del usuario 
        switch (opcion) {
